...
function StoriesAdmin() {
  ...
  const togglePub = useMutation({
    mutationFn: async (row: { id: string; status: string }) => {
      ...
      const { error } = await supabase.from("success_stories").update({ status: next }).eq("id", row.id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="display-h2">Success Stories</h1>
          <p className="mt-2 text-muted-foreground">Manage published case studies.</p>
        </div>
        <Button onClick={() => navigate({ to: "/admin/success-stories/new" })}>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">No stories yet.</td></tr>
            ) : (
              filtered.map((b) => (
                <tr key={b.id} className="border-t border-border">
                  <td className="p-4">
                    <Link to="/admin/success-stories/$id" params={{ id: b.id }} className="font-medium hover:text-primary">
                      {b.title}
                    </Link>
                  ...
                  <td className="p-4 text-right space-x-1 whitespace-nowrap">
                    ...
                    <Button size="sm" variant="ghost" asChild>
                      <Link to="/admin/success-stories/$id" params={{ id: b.id }}>
                        <Pencil className="h-4 w-4" />
                      </Link>
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => { if (confirm("Delete this story?")) remove.mutate(b.id); }}>
